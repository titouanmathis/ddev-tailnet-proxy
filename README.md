...
```toml
[tools]
"github:titouanmathis/ddev-tailnet-proxy" = "0.1.5"
```
...
```sh
sha256sum -c ddev-tailnet-proxy-0.1.5.tar.gz.sha256
tar -xzf ddev-tailnet-proxy-0.1.5.tar.gz
sudo install -m 755 ddev-tailnet-proxy-0.1.5/bin/ddev-tailnet-proxy /usr/local/bin/ddev-tailnet-proxy
sudo ddev-tailnet-proxy service install
```
...
This creates `dist/ddev-tailnet-proxy-0.1.5.tar.gz` and a checksum file that verifies when downloaded beside the archive. Upload both files to a GitHub release with the matching tag.
